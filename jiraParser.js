...
const parseIssues = async () => {
  ...
  const filteredIssuesWithBugsArr = issuesWithCommentsArr.map((issueWithComment) => ({
    key: issueWithComment.key,
    bugsCount: issueWithComment.comments.flatMap((comment) => dataUtils.filterCommentsWithStatuses(comment)).length
  }))
  .filter((issueWithComment) => issueWithComment.bugsCount > 0);
  
  console.log(filteredIssuesWithBugsArr.length);

  let overallBugsCount = 0;

  filteredIssuesWithBugsArr.forEach((issueWithBug) => overallBugsCount += issueWithBug.bugsCount);
